...
class SnakeGameAI:
    ...
    def reset(self):
        ...
        self.food = None
        self._place_food()
        self.frame_iteration = 0 # Track of the iterations of the game
    ...
    def _move(self, action):
        ...
        idx = clock_wise.index(self.direction) # The current direction

        if np.array_equal(action, [1, 0, 0]): # going straight
            new_direction = clock_wise[idx] # no change in direction
        elif np.array_equal(action, [0, 1, 0]): # turning right
            next_idx = (idx+1) % 4  # taking % so that the last position 3 +1 would change to 0 
            new_direction = clock_wise[next_idx] # right turn r -> d -> l -> u
        else: # [0, 0, 1] turning left
            next_idx = (idx-1) % 4
